...
function queryFor(type,query){const filter={};const search=String(query.search||'').trim();if(search)filter.$or=[{title:{$regex:search,$options:'i'}},{summary:{$regex:search,$options:'i'}},{description:{$regex:search,$options:'i'}},{location:{$regex:search,$options:'i'}}];if(query.published==='true')filter.published={$ne:false};if(query.published==='false')filter.published=false;if(type==='jobs'){if(query.type)filter.type=query.type;if(query.location)filter.location={$regex:String(query.location),$options:'i'}}return filter}
export async function list(type,req,res){const page=Math.max(1,Number(req.query.page)||1);const pageSize=Math.min(50,Math.max(1,Number(req.query.pageSize)||10));const filter=queryFor(type,req.query);const collection=await getCollection(type);const total=await collection.countDocuments(filter);const items=await collection.find(filter).sort({createdAt:-1}).skip((page-1)*pageSize).limit(pageSize).toArray();res.json({data:items,pagination:{page,pageSize,total,totalPages:Math.max(1,Math.ceil(total/pageSize))}})}
...
export async function downloadApplicationCv(req,res){
  ...
  const item=await(await getCollection('applications')).findOne({_id:id});
  if(!item)return res.status(404).json({error:'Application not found'});
  const cvUrl=item.cvUrl||item.cv||item.resumeUrl||item.resume;
  if(!cvUrl)return res.status(404).json({error:'CV not found'});
  const originalName=String(item.cvOriginalName||item.originalFilename||item.cvFilename||'CV').trim();
  const sourceName=originalName.includes('.')?originalName:`${originalName}.pdf`;
  const safeName=sourceName.replace(/[^a-zA-Z0-9._-]+/g,'-');
  const mime=String(item.cvMimeType||item.mimetype||'application/pdf');
