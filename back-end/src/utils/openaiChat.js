...
export function getSupportChatProviderInfo() {
  const config = providerConfig();
  return {
...
export async function generateOpenAiChatReply({ history = [], message }) {
  ...
  const recentHistory = history
    .filter(
      (item) =>
        ["visitor", "admin", "bot"].includes(item?.sender) && item?.text,
    )
    .slice(-8)
    .map((item) => ({
      type: "message",
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `${config.provider} request failed (${response.status}): ${payload.slice(0, 180)}`,
    );
  }
