...
export async function generateOpenAiChatReply({ history = [], message }) {
  ...
  const recentHistory = history
    ...
    .map((item) => ({
      type: "message",
      role: item.sender === "visitor" ? "user" : "assistant",
  ...
  const response = await fetch(RESPONSES_URL, {
    ...
    body: JSON.stringify({
      ...
      store: false,
      instructions:
        "Bạn là trợ lý hỗ trợ khách truy cập website SPG. Trả lời ngắn gọn bằng ngôn ngữ của khách. Chỉ hỗ trợ điều hướng website, tuyển dụng, hồ sơ ứng tuyển, dịch vụ logistics và liên hệ. Không được tự bịa địa chỉ, số điện thoại, giá, chính sách, thành tích, đối tác hoặc dữ liệu doanh nghiệp chưa có trong hội thoại. Nếu thiếu dữ liệu cụ thể, nói rõ admin SPG sẽ phản hồi tiếp. Không yêu cầu hoặc tiết lộ mật khẩu, token hay khóa API.",
      input,
