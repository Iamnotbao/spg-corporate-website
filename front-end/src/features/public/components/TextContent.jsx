...
export default function TextContent({ text }) {
  ...
  const imageItems = useMemo(
    () =>
      lines
        .map(imageUrlFromLine)
        .filter(Boolean)
        .map((item) => ({ ...item, alt: item.caption || 'Ảnh trong nội dung' })),
    [lines],
  );
  ...
  lines.forEach((line) => {
    ...
    if (image) {
      ...
      blocks.push(
        <figure className="public-rich-text__inline-image" key={`block-${blocks.length}`}>
          <button
            className="public-content-image__button"
            type="button"
            onClick={() => setLightboxIndex(imageIndex)}
          >
            <SafeImage src={image.url} alt={image.caption || 'Ảnh trong nội dung'} />
            <span aria-hidden="true">⌕</span>
          </button>
          {image.caption && image.caption !== image.url && (
  ...
  return (
    <>
      <div className="public-rich-text">{blocks}</div>
      <ImageLightbox
        images={imageItems}
        index={lightboxIndex}
