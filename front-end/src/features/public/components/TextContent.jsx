...
export default function TextContent({ text }) {
  ...
  function flushList() {
    ...
    blocks.push(type === 'numbered'
      ? <ol className={className} key={`block-${blocks.length}`} start={start === 1 ? undefined : start}>{items.map((item, index) => <li key={`item-${index}`}>{item}</li>)}</ol>
      : <ul className={className} key={`block-${blocks.length}`}>{items.map((item, index) => <li key={`item-${index}`}>{item}</li>)}</ul>);
    activeList = null;
  }
  function addListItem(type, item, start = 1) {
    if (!activeList || activeList.type !== type) { flushList(); activeList = { items: [], start, type }; }
  ...
  lines.forEach((line) => {
    ...
    if (image) {
      flushList();
      blocks.push(<figure className="public-rich-text__inline-image" key={`block-${blocks.length}`}><SafeImage src={image.url} alt={image.caption || 'Ảnh trong nội dung'} />{image.caption && image.caption !== image.url && <figcaption>{image.caption}</figcaption>}</figure>);
      return;
    }
    ...
    if (inlineLabel && !/^https?$/iu.test(inlineLabel[1].trim())) {
      blocks.push(<p className="public-rich-text__paragraph public-rich-text__inline-label" key={`block-${blocks.length}`}><strong>{inlineLabel[1].trim()}:</strong> <span>{inlineLabel[2].trim()}</span></p>);
      return;
    }
