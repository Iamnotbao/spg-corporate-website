...
export default function VideosPage() {
  ...
  const load = useCallback((signal) => {
    ...
    listVideos({ hskLevel: level, page, pageSize: 6, signal })
      .then((response) => setState({ status: 'ready', data: response.data || [], pagination: response.pagination, error: '' }))
      .catch((error) => {
    ...
    <>
      ...
      <section className="video-catalog">
        <div className="public-container">
          <div className="filter-chips" role="group" aria-label="Lọc video theo HSK">
            {['', 'HSK 1', 'HSK 2', 'HSK 3', 'HSK 4', 'HSK 5', 'HSK 6'].map((value) => (
              <button
          ...
          {state.status === 'loading' && <LoadingState count={4} label="Đang tải video" />}
          {state.status === 'error' && <ErrorState message={state.error} onRetry={() => load()} />}
          {state.status === 'ready' && !state.data.length && <EmptyState icon="影" title="Chưa có video" description="Video đã xuất bản sẽ xuất hiện tại đây." />}
          {state.status === 'ready' && state.data.length > 0 && (
            <>
              <div className="video-grid">
                {state.data.map((video) => (
                  <article className="video-card" key={video.id}>
                    <div className="video-player"><VideoPlayer video={video} /></div>
                    <div className="video-card__copy">
                      <span>{video.hskLevel}</span>
                      <h2>{video.title}</h2>
