...
export default function SiteProfilePanel({ onNotify, onUnauthorized }) {
  ...
  function updateMetric(index, field, value) { setForm((current) => ({ ...current, metrics: current.metrics.map((item, itemIndex) => itemIndex === index ? { ...item, [field]: value } : item) })); }
  function updatePartner(index, field, value) { setForm((current) => ({ ...current, partners: current.partners.map((item, itemIndex) => itemIndex === index ? { ...item, [field]: value } : item) })); }
  ...
  return (
    <section className="admin-panel site-profile-admin">
      ...
      <div className="admin-form-section">
        ...
        <div className="site-profile-admin__rows">{form.metrics.map((item, index) => <div className="site-profile-admin__row" key={item.id || index}><input type="number" min="0" value={item.value ?? 0} onChange={(e) => updateMetric(index, 'value', Number(e.target.value))} placeholder="1000" /><input value={item.suffix || ''} onChange={(e) => updateMetric(index, 'suffix', e.target.value)} placeholder="+ / người / đôi" /><input value={item.label || ''} onChange={(e) => updateMetric(index, 'label', e.target.value)} placeholder="Lao động" /><input value={item.note || ''} onChange={(e) => updateMetric(index, 'note', e.target.value)} placeholder="Mô tả ngắn" /><label><input type="checkbox" checked={item.enabled !== false} onChange={(e) => updateMetric(index, 'enabled', e.target.checked)} /> Hiện</label><button type="button" onClick={() => setForm((current) => ({ ...current, metrics: current.metrics.filter((_, i) => i !== index) }))}>×</button></div>)}</div>
      ...
      <div className="admin-form-section">
        ...
        <div className="site-profile-admin__partners">{form.partners.map((item, index) => <article key={item.id || index}><div className="site-profile-admin__logo-preview">{item.logoUrl ? <img src={item.logoUrl} alt={item.name || 'Logo'} /> : <span>LOGO</span>}</div><input value={item.name || ''} onChange={(e) => updatePartner(index, 'name', e.target.value)} placeholder="Tên đối tác" /><input value={item.link || ''} onChange={(e) => updatePartner(index, 'link', e.target.value)} placeholder="https://... (tùy chọn)" /><input value={item.logoUrl || ''} onChange={(e) => { updatePartner(index, 'logoUrl', e.target.value); updatePartner(index, 'logoPublicId', ''); }} placeholder="URL logo" /><div className="site-profile-admin__media-actions"><label className="admin-button admin-button--secondary">{uploadingIndex === index ? 'Đang tải…' : 'Upload logo'}<input hidden type="file" accept="image/*" onChange={(e) => { uploadLogo(index, e.target.files?.[0]); e.target.value = ''; }} /></label><button className="admin-button admin-button--secondary" type="button" onClick={() => setPickerIndex(index)}>Chọn thư viện</button></div><label><input type="checkbox" checked={item.enabled !== false} onChange={(e) => updatePartner(index, 'enabled', e.target.checked)} /> Hiện</label><button type="button" onClick={() => setForm((current) => ({ ...current, partners: current.partners.filter((_, i) => i !== index) }))}>Xóa</button></article>)}</div>
      ...
      <div className="admin-form-section">
        ...
        <div className="admin-form-grid"><label className="admin-form-field"><span>Tên địa điểm</span><input value={form.location?.name || ''} onChange={(e) => updateLocation('name', e.target.value)} placeholder="Tên địa điểm hiện có" /></label><label className="admin-form-field"><span>Google Maps URL mở ngoài</span><input type="url" value={form.location?.mapsUrl || ''} onChange={(e) => updateLocation('mapsUrl', e.target.value)} placeholder="https://maps.app.goo.gl/..." /></label></div>
      ...
      <MediaPicker open={pickerIndex >= 0} onClose={() => setPickerIndex(-1)} onUnauthorized={onUnauthorized} onSelect={(asset) => { if (pickerIndex < 0) return; setForm((current) => ({ ...current, partners: current.partners.map((item, index) => index === pickerIndex ? { ...item, logoUrl: asset.url, logoPublicId: asset.publicId } : item) })); }} />
    </section>
  );
}
