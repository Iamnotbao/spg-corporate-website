...
const EMPTY_FORM = {
  username: '', displayName: '', password: '', role: 'employee',
...
export default function UsersPanel({ currentUser, onNotify, onUnauthorized }) {
  ...
  function beginEdit(user) {
    ...
    setForm({
      ...
      password: '',
      role: user.role,
      permissions: user.role === 'admin' ? ['*'] : (Array.isArray(user.permissions) ? user.permissions : DEFAULT_EMPLOYEE_PERMISSIONS),
      active: user.active !== false,
    });
    setError('');
  }

  function resetForm() { setEditingId(''); setForm(EMPTY_FORM); }
  ...
  return (
    <section className="admin-panel admin-users-panel">
      <div className="admin-panel__heading">
        <div><h2>Quản lý người dùng</h2><p>Phân quyền theo từng mã thao tác, có tìm kiếm và phân trang.</p></div>
        <button className="admin-button admin-button--secondary" disabled={loading} onClick={() => load()} type="button"><AdminIcon name="refresh" size={17} /> Làm mới</button>
      </div>
      {error && <AdminAlert>{error}</AdminAlert>}

      <div className="admin-users-toolbar">
        <label><AdminIcon name="search" size={17} /><input onChange={(event) => setFilters((current) => ({ ...current, search: event.target.value }))} placeholder="Tìm tên đăng nhập hoặc tên hiển thị…" value={filters.search} /></label>
        <select onChange={(event) => setFilters((current) => ({ ...current, role: event.target.value }))} value={filters.role}><option value="">Tất cả quyền</option><option value="admin">Admin</option><option value="employee">Employee</option></select>
        <select aria-label="Số người dùng mỗi trang" onChange={(event) => setPagination((current) => ({ ...current, pageSize: Number(event.target.value), page: 1 }))} value={pagination.pageSize}>{[5, 10, 20, 50].map((size) => <option key={size} value={size}>{size}/trang</option>)}</select>
      </div>

      <div className="admin-users-layout">
        <div className="admin-users-list">
          <div className="admin-users-list__head"><span>Tài khoản</span><span>Vai trò</span><span>Trạng thái</span><span /></div>
          {loading ? <div className="admin-users-empty"><span className="admin-spinner" /> Đang tải người dùng…</div> : users.length ? users.map((user) => (
            <article className={`admin-user-row${editingId === user.id ? ' is-editing' : ''}`} key={user.id}>
              <div><strong>{user.displayName || user.username}</strong><small>@{user.username}{user.id === currentUser?.id ? ' · Bạn' : ''} · {user.role === 'admin' ? 'Toàn quyền' : `${user.permissions?.length || 0} quyền`}</small></div>
              ...
              <div className="admin-user-row__actions"><button className="admin-icon-button" onClick={() => beginEdit(user)} type="button" aria-label={`Sửa ${user.username}`}><AdminIcon name="edit" size={17} /></button><button className="admin-icon-button" disabled={user.id === currentUser?.id} onClick={() => handleDelete(user)} type="button" aria-label={`Xóa ${user.username}`}><AdminIcon name="trash" size={17} /></button></div>
            </article>
          )) : <div className="admin-users-empty">Không tìm thấy tài khoản phù hợp.</div>}
          <div className="admin-users-pagination"><span>{pagination.total} tài khoản</span><div><button disabled={pagination.page <= 1 || loading} onClick={() => load(pagination.page - 1)} type="button">←</button><strong>{pagination.page} / {pagination.totalPages}</strong><button disabled={pagination.page >= pagination.totalPages || loading} onClick={() => load(pagination.page + 1)} type="button">→</button></div></div>
        </div>
        ...
        <form className="admin-user-form" onSubmit={handleSubmit}>
          ...
          <div className="admin-user-form__grid">
            ...
            <label className="admin-form-field"><span>Vai trò</span><select disabled={saving} onChange={(event) => updateField('role', event.target.value)} value={form.role}><option value="employee">Employee</option><option value="admin">Admin</option></select></label>
          </div>

          <div className="admin-permission-editor">
            <div><strong>Mã quyền thao tác</strong><small>{form.role === 'admin' ? 'Admin luôn có toàn quyền.' : 'Có thể cấp riêng từng tab và từng hành động.'}</small></div>
            {PERMISSION_GROUPS.map(([group, permissions]) => (
              <fieldset key={group} disabled={form.role === 'admin' || saving}>
                <legend>{group}</legend>
                <div>{permissions.map((permission) => (
                  <label key={permission}><input type="checkbox" checked={form.role === 'admin' || form.permissions.includes(permission)} onChange={() => togglePermission(permission)} /><span>{permission}</span></label>
                ))}</div>
              </fieldset>
            ))}
          </div>
