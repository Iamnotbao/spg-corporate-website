...
import AdminHskExamPanel from './components/AdminHskExamPanel.jsx';
import AdminVideoPanel from './components/AdminVideoPanel.jsx';
...
import { canAccessAdminSection, findAdminSectionByKey, findAdminSectionByPath } from './navigation.js';
...
export default function AdminApp() {
  ...
  const openCreatePost = useCallback(() => { setEditor({ type: 'posts', item: null }); setDetail(null); }, []);
  const openEditPost = useCallback((item) => { setEditor({ type: 'posts', item }); setDetail(null); }, []);
  ...
  let page;
  if (!activeRoute) page = <AdminRouteState onDashboard={() => navigate('dashboard')} />;
  else if (!canAccessRoute) page = <AdminRouteState accessDenied onDashboard={() => navigate('dashboard')} />;
  else if (editor) page = <ContentEditor item={editor.item} key={`posts-${editor.item?._id?.$oid || editor.item?._id || 'new'}`} onBack={() => setEditor(null)} onSave={handleSave} onUnauthorized={auth.handleUnauthorized} type="posts" />;
  ...
  else if (section === 'hsk-exams') page = <AdminHskExamPanel onNotify={notify} onUnauthorized={auth.handleUnauthorized} />;
  else if (section === 'videos') page = <AdminVideoPanel onNotify={notify} onUnauthorized={auth.handleUnauthorized} />;
  ...
  else if (section === 'homepage') page = <SiteProfilePanel onNotify={notify} onUnauthorized={auth.handleUnauthorized} />;
  else if (section === 'blog') page = <ContentWorkspace key="posts" onCreate={openCreatePost} onEdit={openEditPost} onNotify={notify} onUnauthorized={auth.handleUnauthorized} onView={(item) => setDetail({ item, type: 'posts' })} type="posts" />;
  else if (section === 'media') page = <MediaLibraryPanel onNotify={notify} onUnauthorized={auth.handleUnauthorized} />;
  ...
  return (
    <AdminLayout activeSection={editor ? 'blog' : section} currentUser={auth.user} headerTitle={headerTitle} onLogout={auth.logout} onNavigate={navigate}>
      {page}
      {detail && <ContentDetailModal item={detail.item} onClose={() => setDetail(null)} onEdit={openEditPost} type="posts" />}
