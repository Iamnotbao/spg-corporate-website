...
export function getItemSummary(item) {
  return item?.summary || item?.excerpt || item?.description || 'Chưa có nội dung tóm tắt.';
}
...
export function isUnauthorized(error) { return error?.status === 401 || error?.status === 403; }
export function getErrorMessage(error, fallback = 'Đã có lỗi xảy ra.') { if (error?.name === 'AbortError') return ''; return error?.message || fallback; }
...
function galleryPayload(form) {
  ...
  const imagePublicIds = Array.isArray(form.imagePublicIds) ? form.imagePublicIds.slice(0, images.length) : [];
  return { images, imagePublicIds };
}

function blockPayload(form) {
  const blocks = Array.isArray(form.contentBlocks)
    ? form.contentBlocks.filter((block) => block && typeof block === 'object' && block.type).map((block) => {
        if (block.type === 'paragraph' || block.type === 'heading') return { id: String(block.id || ''), type: block.type, text: String(block.text || '').trim() };
        if (block.type === 'image') return { id: String(block.id || ''), type: 'image', url: String(block.url || '').trim(), publicId: String(block.publicId || '').trim(), caption: String(block.caption || '').trim() };
...
function translationPayload(form, type) {
  const source = form.translations && typeof form.translations === 'object' ? form.translations : {};
  const output = {};
  for (const code of ['en', 'zh-tw']) {
    const value = source[code] || {};
    const common = { title: String(value.title || '').trim(), summary: String(value.summary || '').trim() };
    output[code] = type === 'posts'
      ? { ...common, content: String(value.content || '').trim() }
      : {
          ...common,
          description: String(value.description || '').trim(),
          location: String(value.location || '').trim(),
          salary: String(value.salary || '').trim(),
          benefits: String(value.benefits || '').trim(),
