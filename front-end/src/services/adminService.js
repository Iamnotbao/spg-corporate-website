...
export async function listAdminUsers() {
  return apiRequest('/admin/users', { method: 'GET', auth: true });
}
...
export async function getAdminBanner() {
  return apiRequest('/admin/communications/banner', { method: 'GET', auth: true });
}
...
export async function listAdminNotifications() {
  return apiRequest('/admin/communications/notifications', { method: 'GET', auth: true });
}
...
export async function listAdminContent(type, options = {}) {
  ...
  if (type === 'jobs' && options.location) params.set('location', options.location);
...
export async function createAdminContent(type, payload) {
  assertContentType(type);
  return apiRequest(`/admin/${type}`, { method: 'POST', auth: true, body: payload });
}
...
export async function deleteAdminContent(type, id) {
  assertContentType(type);
  return apiRequest(`/admin/${type}/${encodeURIComponent(id)}`, { method: 'DELETE', auth: true });
}
...
function filenameFromDisposition(disposition) {
  ...
  if (utf8Match) {
    try { return decodeURIComponent(utf8Match[1]); } catch { return ''; }
  }
...
function safeDownloadFilename(value, mime = '') {
  ...
  const safeName = String(value || 'ung-vien-CV').replace(/\\/g, '/').split('/').pop()
    .replace(/[\p{Cc}\\/:*?"<>|]+/gu, '-').replace(/[. ]+$/g, '').trim();
  return /\.(pdf|doc|docx)$/i.test(safeName) ? safeName : `${safeName || 'ung-vien-CV'}.${extension}`;
}

export async function downloadAdminApplicationCv(id) {
  const token = getAdminToken();
  const response = await fetch(`${API_URL}/admin/applications/${encodeURIComponent(id)}/cv`, {
  ...
  const filename = safeDownloadFilename(filenameFromDisposition(response.headers.get('Content-Disposition')), blob.type);
